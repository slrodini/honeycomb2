...
double T_test_5_der(double x1, double x2, double x3)
{
   return -2 * (1 - Power(x1, 2)) * (1 - Power(x2, 2)) * x3
        ...
        + (1 - Power(x1, 2)) * (1 - Power(x2, 2)) * (1 - Power(x3, 2))
              * ((-2 * x2 + 2 * x3) / (Power(x1, 2) + Power(x2, 2) + Power(x3, 2))
                 - Pi * x1 * Sin(Pi * x1 * x3));
}
                                ...
                                std::FILE *fp)
{
   ...
   const double err_ex_der = std::fabs(exact_der) < 1.0e-10 ? std::fabs(inter_der - exact_der)
   ...
   if (nullptr != fp) {
      ...
      Honeycomb::logger(Honeycomb::Logger::INFO,
                        std::format("{:+.16e}\t{:+.16e}\t{:+.16e}\t{:+.16e}\t{:+.16e}\t{:+.16e}", x123(0),
                                    x123(1), x123(2), err, err_der, err_ex_der));
   }
...
int main()
{
   ...
   Honeycomb::logger(
       ...
       std::format("Estimated kernel sizes in MB:      {:f}",
                   static_cast<double>(fj_size) * static_cast<double>(fj_size) * 8.0 / (1024.0 * 1024.0)));

   //
   // Current honeycomb needs to store at least the 2 indexes for each kernel as int32_t, plus some additional
   // space for efficient parallelization
   //
   Honeycomb::logger(Honeycomb::Logger::INFO, std::format("Compare with current honeycomb MB: {:f}",
   ...
   const double inter_der
       = discr.interpolate_df_dx3_fixed_x1(Honeycomb::RnC::from_x123_to_rhophi(x1, x2, x3), fj);
   Honeycomb::logger(
       Honeycomb::Logger::INFO,
       std::format("Derivative at ({:+.3f}, {:+.3f}, {:+.3f}) => [E, N, I]: [{:+.8e}, {:+.8e}, {:+.8e}]", x1,
                   x2, x3, exact_der, num_der, inter_der));
   ...
   Honeycomb::logger(
       Honeycomb::Logger::INFO,
       std::format("Function at   ({:+.3f}, {:+.3f}, {:+.3f}) => [E, I]:    [{:+.10e}, {:+.10e}]", x1, x2, x3,
                   exact, approx));
   double ave            = 0;
   double ave_der        = 0;
   double ave_ex_der     = 0;
   ...
   for (size_t i = 0; i < nSamples; i++) {
      Honeycomb::RnC::Pair rhophi
          = {Honeycomb::Random::random_uniform(rmin, rmax), Honeycomb::Random::random_uniform(0, 6)};
   ...
   fclose(fp);
   Honeycomb::logger(Honeycomb::Logger::INFO,
                     std::format("Average interpolation time:            {:+.6e} ns.",
                                 fnc_elapsed / static_cast<double>(fnc_count)));

   Honeycomb::logger(Honeycomb::Logger::INFO,
                     std::format("Average derivative interpolation time: {:+.6e} ns.",
