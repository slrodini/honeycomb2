...
std::map<size_t, double> compute_g2_weights(const double &xBj, const Honeycomb::Grid2D &grid,
                                            Honeycomb::ThreadPool &th);
...
int main()
{
   ...
   for (size_t i_x = 0; i_x < sx; i_x++) {
      ...
      std::fprintf(stderr, "Computing... ");
      std::map<size_t, double> weight_results = compute_g2_weights(xBj, grid, th);
...
std::map<size_t, double> compute_g2_weights(const double &xBj, const Honeycomb::Grid2D &grid, Honeycomb::ThreadPool &th)
{
