...
Eigen::VectorXd G2Weights::GetWeights(double xBj, const Grid2D &_grid, double int_e_r,
                                      double int_e_a)
{
   ...
   Eigen::VectorXd weights = Eigen::VectorXd::Zero(_grid.c_size_li);
   if (xBj >= 1.0 - 1.0e-15) {
      return weights;
   }
   ...
   for (size_t k = _grid.grid_angle._delim_indexes[0]; k < _grid.grid_angle._delim_indexes[1];
        k++) {
      for (size_t j = 0; j < _grid.grid_radius.size; j++) {
         th.AddTask([&, j, k](void) -> void {
            auto r_supp = _grid.grid_radius.get_support_weight_aj(j);
            const double xi_min
                = std::max(_grid.grid_radius._d_info.to_phys_space(r_supp.first), xBj);
            const double xi_max
                = std::min(_grid.grid_radius._d_info.to_phys_space(r_supp.second), 1.0);
            auto index = _grid.get_flatten_index(j, k);
            size_t c_k = _grid.grid_angle._from_iw_to_ic[k];
            size_t c_j = _grid.grid_radius._from_iw_to_ic[j];
            size_t c_a = _grid.c_get_flatten_index(c_j, c_k);

            auto fnc_external = [&](double xi) -> double {
               auto fnc_internal_1 = [&](double eta) -> double {
                  auto rhophi = RnC::from_x123_to_rhophi(eta, xi - eta, -xi);
                  return -_grid._dw_dx3[index](rhophi) / eta;
               };
   ...
   for (size_t k = _grid.grid_angle._delim_indexes[3]; k < _grid.grid_angle._delim_indexes[4];
        k++) {
      for (size_t j = 0; j < _grid.grid_radius.size; j++) {
         ...
         th.AddTask([&, j, k](void) -> void {
            ...
            auto fnc_external = [&](double xi) -> double {
               auto fnc_internal_1 = [&](double eta) -> double {
                  ...
                  return -(_grid._dw_dx3[index](rhophi)) / (eta + xi);
               };
   ...
   for (size_t k = _grid.grid_angle._delim_indexes[4]; k < _grid.grid_angle._delim_indexes[5];
        k++) {
      for (size_t j = 0; j < _grid.grid_radius.size; j++) {
         th.AddTask([&, j, k](void) -> void {
            auto r_supp = _grid.grid_radius.get_support_weight_aj(j);
            ...
            auto fnc_external = [&](double xi) -> double {
               auto fnc_internal_1 = [&](double eta) -> double {
                  ...
                  return (_grid._dw_dx3[index](rhophi_1) - _grid._dw_dx3[index](rhophi_2)) / eta;
               };
   ...
   for (size_t k = _grid.grid_angle._delim_indexes[5]; k < _grid.grid_angle._delim_indexes[6];
        k++) {
      for (size_t j = 0; j < _grid.grid_radius.size; j++) {
         th.AddTask([&, j, k](void) -> void {
            auto r_supp = _grid.grid_radius.get_support_weight_aj(j);
...
double G2Weights::ComputeSingleQuark(const Eigen::VectorXd &_f) const
{
   return weights.dot(_f);
}
//================================================================================

VectorG2Weights::VectorG2Weights(const Grid2D &_grid, bool to_compute, double int_e_r,
                                 double int_e_a)
    : grid(_grid)
{
...
void VectorG2Weights::GetWeights(double int_e_r, double int_e_a)
{
   for (const double &xBj : grid.grid_radius._coord) {
      logger(Logger::INFO, std::format("Computing g2 weights for xBj={:.4f}", xBj));
      weights.emplace_back(G2Weights::GetWeights(xBj, grid, int_e_r, int_e_a));
   }
}

Eigen::VectorXd VectorG2Weights::interpolate(double xBj)
{
   if (weights.size() == 0) {
      return Eigen::VectorXd::Zero(grid.c_size_li);
   }
   for (size_t j = 0; j < grid.grid_radius._coord.size(); j++) {
      if (is_near(grid.grid_radius._coord[j], xBj)) {
         return weights[j];
      }
   }

   const double r = grid.grid_radius._d_info.to_inter_space(xBj);

   Eigen::VectorXd res = Eigen::VectorXd::Zero(grid.c_size_li);

   for (size_t k = 0; k < grid.grid_radius.size; k++) {
      auto supp = grid.grid_radius.get_support_weight_aj(k);
      if (r < supp.first || r > supp.second) continue;

      res += weights[grid.grid_radius._from_iw_to_ic[k]] * grid.grid_radius._weights[k](r);
   }
