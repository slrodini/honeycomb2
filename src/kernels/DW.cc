...
namespace Honeycomb
{
double DeltaW::integrate(size_t c_a, size_t aP, const Grid2D &g)
{
   ...
   // Yes, x3 is the special point, not x2
   if (std::fabs(x3) < 1.0e-14) {

      auto th_x1_mv_integral = [&](double v) -> double {
         const double w = g._w[aP](RnC::from_x123_to_rhophi(x1 - v, -x1, v));
         return -w;
      };
      ...
      auto th_x1_x3_integral = [&](double v) -> double {
         const double w = g._w[aP](RnC::from_x123_to_rhophi(x1 - v, x2, x3 + v));
         return w * sq(x1) * (x1 + 3 * x3) / cu(x2);
      };
      ...
      if ((x1 > 0 && x3 > 0) || (x1 < 0 && x3 < 0)) {
         double sign  = (x1 > 0) ? +1 : -1;
         result      += sign * integrator::integrate(th_x1_x3_integral, vmin, vmax);
      }
...
double DeltaWP23::integrate(size_t c_a, size_t aP, const Grid2D &g)
{
   const auto &[x1, x2, x3] = g._x123[c_a].v;
   ...
   if (std::fabs(x2) < 1.0e-14) {
      ...
      // Now we can define the lambdas for the kernel
      auto th_x1_x2_integral = [&](double v) -> double {
         const double w = g._w[aP](RnC::from_x123_to_rhophi(x1 - v, x3, x2 + v));
         return w * sq(x1) * (x1 + 3 * x2) / cu(x3);
      };
      ...
      if ((x1 > 0 && x2 > 0) || (x1 < 0 && x2 < 0)) {
         double sign  = (x1 > 0) ? +1 : -1;
         result      += sign * integrator::integrate(th_x1_x2_integral, vmin, vmax);
      }
