...
namespace Honeycomb
{
double Hminus13GG::integrate(size_t c_a, size_t aP, const Grid2D &g)
{
   ...
   if (std::fabs(x2) < 1.0e-14) {
      ...
      auto th_x1_mv_integral = [&](double v) -> double {
         ...
         const double res = w * sq(x1) * (v * x1 + 3 * x3 * v - 2 * x1 * x3) / (cu(x2) * sq(v - x1));
      ...
      auto th_x3_pv_integral = [&](double v) -> double {
         const double w = g._w[aP](RnC::from_x123_to_rhophi(x3 + v, x2, x1 - v));
         return -w * sq(x3) * (v * x3 + 3 * x1 * v + 2 * x1 * x3) / (cu(x2) * sq(v + x3));
      };
