...
namespace Honeycomb
{
double Vminus13::integrate(size_t c_a, size_t aP, const Grid2D &g)
{
   ...
   if (std::fabs(x2) < 1.0e-14) {
      ...
      auto th_x1_mv_integral = [&](double v) -> double {
         ...
         const double f = -x1 * (sq(x1) - x1 * x3 + 2 * v * x3) / (cu(x2) * sq(x1 - v));
      ...
      auto th_x3_pv_integral = [&](double v) -> double {
         const double w = g._w[aP](RnC::from_x123_to_rhophi(x3 + v, x2, x1 - v));
         const double f = -x3 * (-sq(x3) + x1 * x3 + 2 * v * x1) / (cu(x2) * sq(x3 + v));
