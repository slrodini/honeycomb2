...
namespace Honeycomb
{
double Hminus23::integrate(size_t c_a, size_t aP, const Grid2D &g)
{
   ...
   // x1 is fixed => if outside the support just return 0
   if (x1 < x1min || x1 > x1max) return 0;

   const double vmin = std::max(x3 - x2max, x3min - x2);
   const double vmax = std::min(x3 - x2min, x3max - x2);

   // If support is inverted, then we are outside of it, return 0
   if (vmin >= vmax) return 0;

   if (std::fabs(x1) < 1.0e-14) {

      auto th_x3_mv_integral = [&](double v) -> double {
         const double w   = g._w[aP](RnC::from_x123_to_rhophi(0, -x2 - v, x2 + v));
         const double res = w * sq(v) / (2.0 * cu(x3 - v));
      ...
      if (x3 > 0 && vmin <= 0) {
         ...
         result       -= integrator::integrate(th_x3_mv_integral, lower, vmax);
      }
      ...
      auto th_x3_mv_integral = [&](double v) -> double {
         const double w   = g._w[aP](RnC::from_x123_to_rhophi(x1, x3 - v, x2 + v));
         const double res = w * x3 * (2.0 * x2 * (x3 - v) - x3 * (x2 + v)) / (2 * sq(x3 - v) * sq(x1));
