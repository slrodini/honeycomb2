...
namespace Honeycomb
{
double HplusSum12GG::integrate(size_t c_a, size_t aP, const Grid2D &g)
{
   ...
   if (std::fabs(x3) < 1.0e-14) {
      ...
      if (x1 > 0 && vmin <= 0) {
         ...
         result       += integrator::integrate(th_x1_mv_integral, vmin, upper, 1.0e-10, 1.0e-10);
      } else if (x1 < 0 && vmax >= 0) {
         double lower  = std::max(vmin, 0.0);
         result       -= integrator::integrate(th_x1_mv_integral, lower, vmax, 1.0e-10, 1.0e-10);
      }
      ...
      auto th_x1_mv_integral = [&](double v) -> double {
         ...
         const double f = sq(x1) * (v * (x3 - 2 * x2) + 2 * (sq(x3) + x1 * x2)) / (cu(x3) * sq(v - x1));
      ...
      auto th_x2_pv_integral = [&](double v) -> double {
         const double w = g._w[aP](RnC::from_x123_to_rhophi(x1 - v, x2 + v, x3));
         const double f = -sq(x2) * (-2 * x1 * v - 2 * x1 * x2 + v * x3 - 2 * sq(x3)) / (cu(x3) * sq(v + x2));
      ...
      if (x1 > 0 && vmin < 0) {
         ...
         result -=
             integrator::integrate(th_x1_mv_integral, lower, vmax, 1.0e-10, 1.0e-10); // ! note the - from the Theta
      }

      if (x2 > 0 && vmax > 0) {
         const double lower = std::max(0.0, vmin);

         result += integrator::integrate(th_x2_pv_integral, lower, vmax, 1.0e-10, 1.0e-10);
      } else if (x2 < 0 && vmin < 0) {
         const double upper = std::min(0.0, vmax);

         result -=
             integrator::integrate(th_x2_pv_integral, vmin, upper, 1.0e-10, 1.0e-10); // ! note the - from the Theta
      }
