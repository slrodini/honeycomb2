...
double Hplus23::integrate(size_t c_a, size_t aP, const Grid2D &g)
{
   ...
   const double vmin = std::max(x3 - x3max, x2min - x2);
   const double vmax = std::min(x3 - x3min, x2max - x2);
   ...
   if (std::fabs(x1) < 1.0e-14) {
      ...
      auto th_x3_mv_integral = [&](double v) -> double {
         const double w   = g._w[aP](RnC::from_x123_to_rhophi(0.0, -x3 + v, x3 - v));
         const double res = w * v * (v - 2 * x3) / (2.0 * cu(x3 - v));
      ...
      auto th_x3_mv_integral = [&](double v) -> double {
         const double w   = g._w[aP](RnC::from_x123_to_rhophi(x1, x2 + v, x3 - v));
         const double res = w * x3 * (x2 - x1) / (2 * (x3 - v) * sq(x1));
      ...
      auto th_x2_pv_integral = [&](double v) -> double {
         const double w = g._w[aP](RnC::from_x123_to_rhophi(x1, x2 + v, x3 - v));
         return w * sq(x2) * (x2 - x1 + v) / (2 * sq(x2 + v) * sq(x1));
      };
