...
namespace Honeycomb
{
double Hminus12GG::integrate(size_t c_a, size_t aP, const Grid2D &g)
{
   ...
   if (std::fabs(x3) < 1.0e-14) {
      ...
      auto th_x1_mv_integral = [&](double v) -> double {
         ...
         const double res = w * sq(x1) * (v * x1 + 3 * x2 * v - 2 * x1 * x2) / (cu(x3) * sq(v - x1));
      ...
      auto th_x2_pv_integral = [&](double v) -> double {
         const double w = g._w[aP](RnC::from_x123_to_rhophi(x2 + v, x1 - v, x3));
         return -w * sq(x2) * (v * x2 + 3 * x1 * v + 2 * x1 * x2) / (cu(x3) * sq(v + x2));
      };
