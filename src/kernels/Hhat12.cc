...
double Hhat12::subtracted_integrate(size_t c_a, size_t aP, const Grid2D &g)
{
   ...
   const double subtr = g._w[aP](RnC::from_x123_to_rhophi(x1, x2, x3));

   // Now we can define the lambdas for the kernel
   auto th_x1_mv_integral = [&](double v) -> double {
      const double w   = g._w[aP](RnC::from_x123_to_rhophi(x1 - v, x2 + v, x3)) - subtr;
      const double res = w * x1 / (v * (x1 - v));
   ...
   auto th_x2_pv_integral = [&](double v) -> double {
      const double w = (x2 / (x2 + v)) * g._w[aP](RnC::from_x123_to_rhophi(x1 - v, x2 + v, x3)) - subtr;
      return -x2 * w / (v * (x2 + v));
   };
   ...
   if (x2 == 0.0) result += g._w[aP](RnC::from_x123_to_rhophi(x1, 0, -x1));
   if (x2 >= 0 && vmax > 0) {
      const double lower = std::max(0.0, vmin);
