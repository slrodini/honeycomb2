...
double Hhat31GG::subtracted_integrate(size_t c_a, size_t c_aP, const Grid2D &g)
{
   ...
   for (size_t c_h = 0; c_h < g.grid_angle._from_ic_to_iw[c_i].size(); c_h++) {
      for (size_t c_l = 0; c_l < g.grid_radius._from_ic_to_iw[c_j].size(); c_l++) {
         ...
         double loc_vmin = std::max(x3 - x3max, x1min - x1);
         double loc_vmax = std::min(x3 - x3min, x1max - x1);
   ...
   auto th_x1_pv_integral = [&](double v) -> double {
      double w = 0;
      for (const long int &q : w_indexes)
         w += g._w[q](RnC::from_x123_to_rhophi(x1 + v, x2, x3 - v));
   ...
   if (x1 >= 0 && vmax > 0) {
      ...
      result += integrator::integrate(th_x1_pv_integral, lower, vmax);
      if (std::fabs(subtr - 1.0) < 1.0e-12) result += log(1 + x1 / vmax);

   } else if (x1 <= 0 && vmin < 0) {
      const double upper = std::min(0.0, vmax);

      result -= integrator::integrate(th_x1_pv_integral, vmin, upper); // ! note the - from the Theta
      if (std::fabs(subtr - 1.0) < 1.0e-12) result += log(1 + x1 / vmin);
   }
